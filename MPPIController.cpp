...
bool MPPIController::loadConfig(const std::string &filename) {
    ...
    while (std::getline(in, line)) {
        ...
        double value;
        iss >> key >> value;
        if (key == "K") K = static_cast<int>(value);
        else if (key == "N") N = static_cast<int>(value);
        else if (key == "lambda") lambda = value;
        else if (key == "Sigma00") Sigma(0,0) = value;
        else if (key == "Sigma11") Sigma(1,1) = value;
        else if (key == "pos_cost_weight") pos_cost_weight = value;
        else if (key == "vel_cost_weight") vel_cost_weight = value;
        else if (key == "delta_cost_weight") delta_cost_weight = value;
        else if (key == "collision_cost_weight") collision_cost_weight = value;
        else if (key == "obstacle_eps") obstacle_eps = value;
        else if (key == "max_delta") max_delta = value;
        else if (key == "max_Fx") max_Fx = value;
    }
...
Eigen::Vector2d MPPIController::computeControl(const Eigen::VectorXd& x_init, const Eigen::VectorXd& reference) {
    ...
    for (int k = 0; k < K; ++k) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            model->Predict(x_sim, u_noisy, x_next); // call into your Predict implementation
            // if state blows up
            ...
            cost_k += computeRunningCost(x_sim, reference);
    ...
    if (weight_sum <= 0) weight_sum = 1e-9;
    ...
    for (int i = 0; i < N - 1; ++i) U.col(i) = U.col(i+1);
...
{
    if (x.hasNaN()||ref.hasNaN()) return 1e12;
    ...
    double cost = pos_cost_weight*pos_cost            // stronger tracking
                ...
                + delta_cost_weight*std::abs(x(2)-ref(2));
